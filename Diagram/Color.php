...
{	protected $htmlColors = array( 
	...
	public function __Construct($color)
	{
		if( is_array($color) && count($color) == 3 )
	        ...
	        throw new Exception\ColorException("Color array was not valid.");

		if( $c = $this->GetColor($color) )
			$this->color = array($c["red"], $c["green"], $c["blue"]);
		else
	...
	{
		if( stristr(get_class($adapter), "ImagickAdapter") )
		{
			return "rgb(".$this->color[0].",".$this->color[1].",".$this->color[2].")";
		}
		else if( stristr(get_class($adapter), "GDAdapter") )
		{
			return imagecolorallocate($adapter->adapter, $this->color[0], $this->color[1], $this->color[2]);
		}
		return $this->color;
	}
	...
	private function html2rgb($color)
	{
	    ...
	    {
	        throw new Exception\ColorExcetion("Html color was not valid.");
	    }
    ...
    {
    	if( isset( $this->htmlColors[$colorName] ) )
        	return $this->htmlColors[$colorName];
        else
        	return null;
    }
}
